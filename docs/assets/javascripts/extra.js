...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const defaultStepHandler = (target, name, step) => {
    ...
    target.href = `${target.href}?${name}=${param}&ref=${ref}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // there is no core on GitHub
  if (ref === "core") {
    ref = "installation";
  }

  // This creates an HTML structure <strong><code>ParamValue</code></strong>, <strong..
  ...
  for (let i = 0; i < param.length; i++) {
    ...
    strongElement.appendChild(codeElement);
    newNodes.push(strongElement);
    // For step 1 create a download release link based on the ref
    if (ref && i === 0 && step === 1) {
      const anchor = document.createElement("a");
      anchor.href = `${releaseBase}/${ref}/releases`;
      anchor.appendChild(strongElement);
      newNodes[i] = anchor;
    }
  ...
  for (const el of internalCodeElements) {
    if (el.innerHTML === placeholder[step]) {
      ...
      elCopy.innerHTML = elCopy.innerHTML.replace(placeholder[step], param[0]);

      // Replace the new <code> with an even newer <code>
      newNodes[0].children[0].replaceWith(elCopy);
